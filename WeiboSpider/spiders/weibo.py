...
class WeiboSpider(CrawlSpider):
    ...
    def parse_post_info(self, response):
        for div_selector in response.xpath('//div[@class="c"]'):
            if div_selector.xpath('@id'):
                ...
                if div_selector.xpath('div[2]'):
                    post_info_item['publish_time'] = div_selector.xpath(
                    ...
                    for a_selector in div_selector.xpath('div[2]/a'):
                        ...
                        if temp_str[:1] == '赞':
                            start_thumbup_url = \
                            re.split(r'(http://weibo.cn/attitude/[^/]+)', a_selector.xpath('@href').extract_first())[1] \
                            + '?#attitude'
                        elif temp_str[:2] == '转发':
                    ...
                    post_info_item['publish_time'] = div_selector.xpath(
                        'div[1]/span[@class="ct"]/text()').extract_first()
                ...
                post_info_item['publish_time'] = str(
                    self.handle_time(self.get_time(response.headers['date']), publish_time))
        ...
        if response.xpath('//div[@class="pa" and @id="pagelist"]') \
                and response.xpath(
                    '//div[@class="pa" and @id="pagelist"]/form/div/a[1]/text()').extract_first() == '下页':
